...
exclude         <- c("date","date_expected","zenit",future,"horizon")
...
toNorm <- all[-c(1:(length(exclude)+1))]
...
inTrainValid <- trainData[,.(byLabel= which(day(date_expected) <= (daysOnlyTrain) )),by=wTypeF]
...
modelErrors <- data.table(foreach(i= methods,.combine=rbind) %do% {
    ...
    model <- trainData[,.(model= list(train( as.formula(paste(target,"~. -", paste(exclude,collapse=" - "),sep="")),
                                             ...
                                             tuneLegnth = 10,
                                             na.action = na.pass,
                                             trControl = trainControl(method = "LGOCV",index = inTrainValid[wTypeF==.BY[[1]],list(a=c(byLabel))] ,number = 1))
    )),
    by=wTypeF]
    ...
    trainPred <- dnrm(model[,predict(m,newdata = trainData[.BY[[1]] == wTypeF,],na.action= na.pass),by=s][,V1],min,max)
    testPred  <- dnrm(model[,predict(m,newdata =  testData[.BY[[1]] == wTypeF,],na.action= na.pass),by=s][,V1],min,max)
    trainObs  <- model[,trainData[.BY[[1]] == wTypeF,c(target,metadata),with=F],by=s][,obs:= dnrm(.SD[[2]],min,max)]
    testObs   <- model[, testData[.BY[[1]] == wTypeF,c(target,metadata),with=F],by=s][,obs:= dnrm(.SD[[2]],min,max)]
