...
predictionType  <- c("Satelite","WRFsolar","SmartPersistence","CIADCast")
...
exclude         <- c("date","date_expected","wTypeF","zenit",future,"horizon")
...
dnrm <- function(v,max,min)  v * (max - min) + min
...
inTrainValid <- trainData[, which(day(date_expected) <= (daysOnlyTrain) )]
...
N <- length(exclude)
meanTrain <- data[ inTrain,c(exclude,target),with=F][,.(horizon= .SD[[N]],target= .SD[[N+1]])][,.(mn= mean(target,na.rm=T)),by=horizon]
meanTest  <- data[-inTrain,c(exclude,target),with=F][,.(horizon= .SD[[N]],target= .SD[[N+1]])][,.(mn= mean(target,na.rm=T)),by=horizon]
if(useKt) {
    meanTrain <- data[ inTrain,c(exclude,target),with=F][,.(future= .SD[[N-1]], horizon= .SD[[N]],target= .SD[[N+1]])][,.(mn= mean(target*future,na.rm=T)),by=horizon]
    meanTest  <- data[-inTrain,c(exclude,target),with=F][,.(future= .SD[[N-1]], horizon= .SD[[N]],target= .SD[[N+1]])][,.(mn= mean(target*future,na.rm=T)),by=horizon]
}

#ENTRENAR TODOS LOS METODOS Y OBTENER ERRORES ########################################################################################
modelErrors <- data.table(foreach(i= methods,.combine=rbind) %do% {
    #ENTRENAMIENTO
    model <- train( as.formula(paste(target,"~. -", paste(exclude,collapse=" - "),sep="")), 
                    trainData,
                    method = i,
                    tuneLegnth = 10,
                    na.action = na.pass,
                    trControl = trainControl(method = "LGOCV", index = list(valid= inTrainValid), number = 1)
    )
    
    #PREDICCIONES Y OBSERVACIONES
    trainErrorTable <- trainData[,c(metadata,target),with=F][,pred:= predict(model, newdata= trainData, na.action= na.pass)]
    testErrorTable  <-  testData[,c(metadata,target),with=F][,pred:= predict(model, newdata=  testData, na.action= na.pass)]
    colnames(trainErrorTable) <- c(metadata,"obs","pred")
    colnames( testErrorTable) <- c(metadata,"obs","pred")
    max <- maxData[target]
    min <- minData[target]
    ...
    csTr <- 1
    if(useKt){
        csTr <- trainData[,future,with=F]
    }
    trainErrorTable[,obs:=dnrm(obs,max,min)*csTr  ][,pred:=dnrm(pred,max,min)*csTr]
    trainErrorTable[,type:="train"][,method:=i]
    #trainErrorTable[,horizon:=dnrm(horizon,maxData["horizon"],minData["horizon"])]

    #DENORMALIZAR SALIDA TEST
    csTe <- 1
    if(useKt){
        csTe <- testData[,future,with=F]
    }
    testErrorTable[ ,obs:=dnrm(obs,max,min)*csTe][ ,pred:=dnrm(pred,max,min)*csTe]
    testErrorTable[,type:="test"][,method:=i]
    #testErrorTable[ ,horizon:=dnrm(horizon,maxData["horizon"],minData["horizon"])]
